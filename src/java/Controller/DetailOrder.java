...
public class DetailOrder extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            Order order = od.getOrderById(id);
            List<OrderDetail> listOrderDetail = od.getOrderDetailsByOrderId(id);
            List<ProductInfo> productInfoList = new ArrayList<>();
            for (OrderDetail orderDetail : listOrderDetail) {
                Product_Size ps = pd.getProduct_SizeById(orderDetail.getProductSizeId());
                if (ps == null) {
                    throw new Exception("Không tìm thấy kích thước sản phẩm (ID: " + orderDetail.getProductSizeId() + ")");
                }
                Product p = pd.getProductById(ps.getProductId());
                ProductInfo pf = new ProductInfo(p, ps, orderDetail.getQuantity(), 0,orderDetail.getPrice());
